...
void ControlUWSim::control_attitude(double dt) {
  ...
  float e_R_x_sin = e_R.norm(); // Vector Geometric length not array length. With sin(angle) = Vector2-Vector1. Small Angle Appx
...
//    Eigen::Vector3f e_R_d = q_error(0) >= 0.0f ? q_error.imag() * 2.0f : -q_error.imag() * 2.0f;
  ...
  /* At this point we've decomposed the rotation error into rpy errors, we now calculate the rates setpoint from the
   * att error: edot = e * Kp */
  _rates_sp = _params.att_p.cwiseProduct(e_R);
